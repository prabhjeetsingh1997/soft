<?php 
	...
	$arrQuery=$objAdmin->getAllQuery($admin_Id, $user_type, $empType, $queryType,$start_from,$record_per_page);
          ...
          <div class="row">
            <div class="col-xs-12">
               ...
               <!--  <div class="box-header">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
					...
					?>
                      <tr>
						...
						<!-- <td><?php //echo $val['query_handled_by']; ?></td> -->
						<!--<td>
						...
						<td style="padding: 2px;">
							<div class="btn-group btn-group-xs">
								...
								if ($userType == 'Employee') { 
									if(!empty($val['query_handled_by'])){
										echo $val['query_handled_by'];
									}else{ ?>
										<a class="btn btn-info" title="Pull Query" data-toggle="tooltip" onclick="pullQuery('<?php echo $val['id']; ?>','<?php echo @$_SESSION['admin_Email'];?>')"><i class="fa fa-sign-out" aria-hidden="true"></i></a>
									<?php } ?>
								<?php }
								...
								{ if(!empty($val['query_handled_by'])){
									...
									<a class="btn btn-info" title="Push Query" data-toggle="tooltip" onclick="pushQuery('<?php echo $val['id']; ?>')"><i class="fa fa-sign-out" aria-hidden="true"></i></a>
								<?php } ?>
    			...
    			<br />
    		<?php
			    $page_result=$objAdmin->getAllQueryResult($admin_Id, $user_type, $empType, $queryType);
			    ...
			    if($page > 1)
			    {
			     ...
			     echo "<li class='queryPage'><a href='view_query.php?page=1'>First</a></li>";
			     echo "<li class='queryPage'><a href='view_query.php?page=".($page - 1)."'><<</a></li>";
			    }
			    for($i=$start_loop; $i<=$end_loop; $i++)
			    {
			     ...
			     echo "<li class='queryPage ".$class."'><a href='view_query.php?page=".$i."'>".$i."</a></li>";
			    }
			    if($page <= $end_loop)
			    {
			     echo "<li class='queryPage'><a href='view_query.php?page=".($page + 1)."'>>></a></li>";
			     echo "<li class='queryPage'><a href='view_query.php?page=".$total_pages."'>Last</a></li>";
			    }
			    echo "</ul>";
...
<div id="eDetail" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header" style="color: #FFF;background: #4357ca;">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title" id="modalcancelHead">Employee List</h4>
      </div>
      	<div class="modal-body" id="edetail">
      	<select id="emplist" class="form-control">
      	</select>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onclick="AssignQuery();">Assign</button>
      </div>
    </div>
...
<script type="text/javascript">
	function pullQuery(query_id,user_id) {
		$.ajax({
			type:'post',
			url : '_ajax_query_pull_request.php',
			data:{id:query_id,userId:user_id},
			beforeSend:function() {
			},
	...
	function pushQuery(query_id) {
		$.ajax({
			type:'post',
			url:'_ajax_employeee_list_request.php?action=getEmp',
			success:function(result) {
				if(result!=null){
					var ret = JSON.parse(result);
					$("#edetail").append('<input type ="hidden" id="query_id" value ="'+query_id+'">')
					$.each(ret, function(index, element){
						$('#emplist').append('<option value="'+element.user_id+'">'+element.first_name+"</option>");	
					});
					$("#eDetail").modal();
				}
			}
	...
	function AssignQuery() {
		...
		$.ajax({
			type:'post',
			url:'_ajax_employeee_list_request.php?action=assignEmp',
			data:{user_id:id,id:query_id},
			success:function(result) {
				if(result === '1')
				{
					window.location.href='view_query.php';
				...
				{
					$("#status").show().html('<div class="alert alert-danger">Sorry, Push request no generated</div>');	
				}
			}
		})
	}
