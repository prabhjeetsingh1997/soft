...
class Admin{
	...
	public function get_countery_aj($id) {
	...
	public function get_city1($id) {
		
		$city_new=explode(",",$id);
		$city_new1=implode(",",$city_new);
		// echo $query ="SELECT * FROM citymaster WHERE statemasterid IN($city_new1)  order by id";exit;
		$query = $this->db->prepare("SELECT * FROM citymaster WHERE statemasterid IN($city_new1)  order by id");
		// $query->bindValue(1, $id);
		try{
				$query->execute();
				return $query->fetchAll(PDO::FETCH_ASSOC);
...
/*	----------------------------------------------Employee Form personal details------------------------------------------*/	
	public function adduser($data)
	{
		...
		$query=$this->db->prepare("insert into admin(name_perfix, first_name, middle_name, last_name, email, father_name_perfix, father_first_name, father_middle_name, father_last_name, mother_name_prefix, mother_first_name, mother_middle_name, mother_last_name, data_of_birth, user_type, ip_address,email_address,partner_url) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		$query->bindValue(18, $partner_url);
		try{
			$query->execute(); 
			return $lastrID = $this->db->lastInsertId();
		}catch(PDOException $e){
...
/*	----------------------------------------------Employee Form Document Detail-----------------------------------------*/	
	...
	public function user_list($domain) {
		$query = $this->db->prepare("SELECT * FROM admin WHERE user_type = 'Employee' and partner_url='$domain' order by admin_id DESC");
		...
		public function add_query($queryData)
	{
		...
		else
		{
			...
			$queryNum = 'lidq00'.$num;
			//$source = $source;
			$query_client_check=$this->db->prepare("SELECT contact_no FROM phone_number where type='client_personal' and partner_url IS NULL");
			$query_client_check->execute();
			$data2=$query_client_check->rowCount();
			$data1 = $query_client_check->fetchAll(PDO::FETCH_COLUMN, 0);
			//print_r($contactNum);exit;
			
			if(!in_array($contactNum,$data1))
			{
				$query2=$this->db->prepare("INSERT INTO clients(first_name, primary_email,additional_email_address) VALUES(?,?,?)");
				$query2->bindValue(1, $prsnName);
				$query2->bindValue(2, $prsnEmail);
				$query2->bindValue(3, $prsnEmail);
                $query2->execute();
                $pan_id=$this->db->lastInsertId();
			     
                $clientId_gen = $this->autogenerate_id($pan_id, 'C');
		        $query3 = $this->db->prepare("UPDATE clients SET client_login_id=? WHERE client_id=?");
				$query3->bindValue(1, $clientId_gen);
				$query3->bindValue(2, $pan_id);
				$query3->execute();
				
				
				$query1=$this->db->prepare("INSERT INTO phone_number(panel_id, panel_name,contact_no,type) VALUES(?,?,?,?)");
				$query1->bindValue(1, $pan_id);
				$query1->bindValue(2, 'client');
				$query1->bindValue(3, $contactNum);
				$query1->bindValue(4, 'client_personal');
				$query1->execute();
	...
	public function login($email,$password,$userType,$partner_url) {
		...
		{
			$query1 = $this->db->prepare("SELECT `email`, `password`,admin_id,user_type,employee_type,user_id FROM `admin` WHERE `user_id` = ? AND password=? AND partner_url=?");
			$query1->bindValue(1, $email);
		$query1->bindValue(2, base64_encode($password));
		$query1->bindValue(3, $partner_url);
		$query1->execute();
	...
	public function getAllTourcard($start_from,$record_per_page)
	{
		$query = $this->db->prepare("SELECT * FROM tour_card ORDER BY id DESC LIMIT $start_from, $record_per_page");
		//echo $query = "SELECT * FROM tour_card WHERE employee_id = '$emp_id' and partner_url ='$domain'  ORDER BY id DESC LIMIT $start_from, $record_per_page";exit;
		// if($emp_id!="swastikholidays@gmail.com")
		// {
	 	// $query = $this->db->prepare("SELECT * FROM tour_card WHERE employee_id = '$emp_id' and partner_url ='$domain'  ORDER BY id DESC LIMIT $start_from, $record_per_page");
        // }
		// else
		// {
			// $query = $this->db->prepare("SELECT * FROM tour_card WHERE partner_url ='$domain'  ORDER BY id DESC LIMIT $start_from, $record_per_page");
		// }
		// $query->bindValue(1, $emp_id);
		// $query->bindValue(2, $domain);
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
	...
	public function getAlltourcardResult()
	{
		$query = $this->db->prepare("SELECT * FROM tour_card ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
	...
	public function getAllOtherTourcard($start_from,$record_per_page)
	{
		$query = $this->db->prepare("SELECT * FROM other_tour_card_details  ORDER BY id DESC LIMIT $start_from, $record_per_page");	
		
		//echo $query = "SELECT * FROM tour_card WHERE employee_id = '$emp_id' and partner_url ='$domain'  ORDER BY id DESC LIMIT $start_from, $record_per_page";exit;
		// if($emp_id!="swastikholidays@gmail.com")
		// {
	 	// $query = $this->db->prepare("SELECT * FROM other_tour_card_details WHERE employee_id = '$emp_id' and partner_url ='$domain' ORDER BY id DESC LIMIT $start_from, $record_per_page");
        // }
		// else
		// {
			
		// $query = $this->db->prepare("SELECT * FROM other_tour_card_details WHERE partner_url ='$domain'  ORDER BY id DESC LIMIT $start_from, $record_per_page");	
		// }
		// $query->bindValue(1, $emp_id);
		// $query->bindValue(2, $domain);
		try{
	...
	public function getAllOthertourcardResult()
	{
		$query = $this->db->prepare("SELECT * FROM other_tour_card_details ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
	...
	public function getHotelFilterName($id)
	{

	   $sql = "SELECT address_details.panel_id,hotels.hotel_id,hotels.hotel_name FROM address_details
		         LEFT JOIN hotels ON address_details.panel_id = hotels.hotel_id
                 where address_details.city='$id' and hotel_id!='null'"; 
	...
	public function getAllotherTourcard_info($id,$queryType)
	{
		//echo $query ="SELECT * FROM tour_card WHERE id = '$per_id' LIMIT 1";exit;
	 	$query = $this->db->prepare("SELECT * FROM other_tour_card_details WHERE id = ? and queryType=? LIMIT 1");
        $query->bindValue(1, $id);
		$query->bindValue(2, $queryType);
		try{
			$query->execute();
			return $query->fetch(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
	...
	public function getpartnerHotel($domain)
	{
		$query=$this->db->prepare("SELECT hotel_id,hotel_name FROM hotels WHERE partner_url=?");
		$query->bindValue(1, $domain);
		try{
			$query->execute();
            return $query->fetchAll(PDO::FETCH_ASSOC);
		    } catch(PDOException $e){
	...
	public function gettravel_name($domain)
	{
		$query=$this->db->prepare("SELECT id,hotal_name as hotel_name FROM travel_agent WHERE partner_url=?");
		$query->bindValue(1, $domain);
		try{
			$query->execute();
            return $query->fetchAll(PDO::FETCH_ASSOC);
...
public function get_hotel_city_filter($id)
		{

	     $sql = "SELECT hotels.hotel_id,hotels.hotel_name FROM hotels
		         LEFT JOIN address_details ON hotels.hotel_id = address_details.panel_id 
                 where address_details.city='$id'"; 
	...
	public function getAllTourcard_info($id,$queryType)
	{
		//echo $query ="SELECT * FROM tour_card WHERE id = '$per_id' LIMIT 1";exit;
	 	$query = $this->db->prepare("SELECT * FROM tour_card WHERE id = ? and queryType=? LIMIT 1");
        $query->bindValue(1, $id);
		$query->bindValue(2, $queryType);
		try{
			$query->execute();
		...
		public function getAlltour_card_details($id)
	{
		//echo $query ="SELECT * FROM tour_card WHERE id = '$per_id' LIMIT 1";exit;
	 	$query = $this->db->prepare("SELECT * FROM tour_card_details WHERE tour_card_id = ? LIMIT 1");
        $query->bindValue(1, $id);
		try{
			$query->execute();
			return $query->fetch(PDO::FETCH_ASSOC);
	...
	public function getAlltour_card_pack_dt($id)
	{
		//echo $query ="SELECT * FROM tour_card WHERE id = '$per_id' LIMIT 1";exit;
	 	$query = $this->db->prepare("SELECT * FROM tour_card_package_dt WHERE tour_card_id = ?");
        $query->bindValue(1, $id);
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		...
		public function getAlltour_card_room($id)
	{
		//echo $query ="SELECT * FROM tour_card WHERE id = '$per_id' LIMIT 1";exit;
	 	$query = $this->db->prepare("SELECT * FROM tour_card_room_dt WHERE tour_card_id = ?");
        $query->bindValue(1, $id);
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
	...
	public function get_partner($url)
	{
		$query = $this->db->prepare("SELECT * FROM partner_register WHERE url_link =?");
	...
	public function get_partner_url_name($id)
	{
		$query = $this->db->prepare("SELECT partner_url  FROM tour_card WHERE id =?");
		$query->bindValue(1,$id);
		try{
			$query->execute();
            return $query->fetch(PDO::FETCH_ASSOC);
	...
	public function get_partner_url_other_name($id)
	{
		$query = $this->db->prepare("SELECT partner_url  FROM other_tour_card_details WHERE id =?");
		$query->bindValue(1,$id);
		try{
			$query->execute();
            return $query->fetch(PDO::FETCH_ASSOC);
   ...
   public function getALLmrsCapitalEvent()
   {

   	    $query = $this->db->prepare("SELECT * FROM mrs_capital ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
   ...
   public function getmrsCapitalLeadDetail($id)
   {
   	    $query = $this->db->prepare("SELECT * FROM mrs_capital WHERE id ='".$id."' ");
   	    //$query->bindValue(1,$id);
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
   ...
   public function deleteMrsCapitalLeadById($id)
    {
    	$query=$this->db->prepare("DELETE from mrs_capital WHERE id=?");
		$query->bindValue(1,$id);
		try{
			$query->execute(); 
			return true;
	...
	{
		$query = $this->db->prepare("SELECT * FROM mrs_capital WHERE id =?");
		$query->bindValue(1,$id);
		try{
			$query->execute();
            return $query->fetchAll(PDO::FETCH_ASSOC);
	...
	{
		// $partner_url='';
		// echo $query = "SELECT first_name,middle_name,last_name,user_id FROM admin WHERE partner_url IS NULL ORDER BY admin_id DESC";exit;
		$query = $this->db->prepare("SELECT first_name,middle_name,last_name,user_id FROM admin WHERE partner_url IS NULL ORDER BY admin_id DESC");
	...
	public function AssignEmployee($user_id,$query_id)
	{
		$query1 = $this->db->prepare("SELECT first_name, middle_name , last_name FROM admin WHERE user_id=?");
		$query1->bindValue(1,$user_id);
		//$query1->bindValue(2,$partner_url);
		try{
				$query1->execute();
				$result = $query1->fetchAll(PDO::FETCH_ASSOC); 
			    } catch(PDOException $e){
	...
	public function getALLDskLeads()
    {
   	    ...
   	    $query = $this->db->prepare("SELECT * FROM dsk_leads_detail  WHERE (photo_received = '0' AND 2nd_payment = '0' AND cancelled = '0') ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function add_new_dsk_event_detail($post)
    {
        extract($post);
        
    	$filename1  =  $_FILES['photo_1']['name'];
    	$file_size1 =  $_FILES['photo_1']['size'];
    	$file_tmp1  =  $_FILES['photo_1']['tmp_name'];
    	$file_type1 =  $_FILES['photo_1']['type'];
    	
    	$filename2  =  $_FILES['photo_2']['name'];
    	$file_size2 =  $_FILES['photo_2']['size'];
    	$file_tmp2  =  $_FILES['photo_2']['tmp_name'];
    	$file_type2 =  $_FILES['photo_2']['type'];
    	
    	$filename3  =  $_FILES['photo_3']['name'];
    	$file_size3 =  $_FILES['photo_3']['size'];
    	$file_tmp3  =  $_FILES['photo_3']['tmp_name'];
    	$file_type3 =  $_FILES['photo_3']['type'];

    	$realpath1 =  'images/mrscapitalimages/'.$filename1;
    	$realpath2 =  'images/mrscapitalimages/'.$filename2;
    	$realpath3 =  'images/mrscapitalimages/'.$filename3;
    	if(file_exists($realpath1)) 
    	{
        	$filename1 = date("mdYHis")."_".$filename1;
        	$realpath1 = 'images/mrscapitalimages/'.$filename1;
    	}

    	move_uploaded_file($file_tmp1,$realpath1);
    	
    	//$realpath2 =  'images/childimages/'.$filename2;
    	if(file_exists($realpath2)) 
    	{
        	$filename2 = date("mdYHis")."_".$filename2;
        	$realpath2 = 'images/mrscapitalimages/'.$filename2;
    	}

    	move_uploaded_file($file_tmp2,$realpath2);
    	
    	//$realpath =  'images/childimages/'.$filename;
    	if(file_exists($realpath3)) 
    	{
        	$filename3 = date("mdYHis")."_".$filename3;
        	$realpath3 = 'images/mrscapitalimages/'.$filename3;
    	}

    	move_uploaded_file($file_tmp3,$realpath3);
    	
    // 	extract($post);
    	date_default_timezone_set("Asia/Kolkata");
    	$dateandtime = date('Y/m/d H:i:s');

    	$query = $this->db->prepare("INSERT INTO mrs_capital(first_name, last_name, age, maritial_status, contact, email, address_line_1, address_line_2, city, state, postal_code, height, weight, bust, waist, hips, photo_1, photo_2, photo_3, payment_status, amount, reference_id, date_time) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$query->bindValue(1, $firstname);
		$query->bindValue(2, $lastname);
		$query->bindValue(3, $age);
		$query->bindValue(4, $martialstatus);
		$query->bindValue(5, $mobilenumber);
		$query->bindValue(6, $email);
		$query->bindValue(7, $address);
		$query->bindValue(8, $address1);
		$query->bindValue(9, $city);
		$query->bindValue(10, $state);
		$query->bindValue(11, $pincode);
		$query->bindValue(12, $height);
		$query->bindValue(13, $weight);
		$query->bindValue(14, $bust);
		$query->bindValue(15, $waist);
		$query->bindValue(16, $hips);
		$query->bindValue(17, $filename1);
		$query->bindValue(18, $filename2);
		$query->bindValue(19, $filename3);
		$query->bindValue(20, 'success');
		$query->bindValue(21, '1770');
		$query->bindValue(22, 'Cash');
		$query->bindValue(23, $dateandtime);
		try{
			$query->execute();
			return $lastrID = $this->db->lastInsertId();
		} catch (PDOException $e){
    ...
    {
    	$csvfile 	= $files["csv"]["name"];
        $csvsource 	= $files["csv"]["tmp_name"];
        $csvtype 	= $files["csv"]["type"];

        $target_path = 'dskcsv/'.$csvfile;
        move_uploaded_file($csvsource, $target_path);
        $row = 0;
        if (($handle = fopen($target_path, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                if ($row > 0) {
                	$query = $this->db->prepare("INSERT INTO dsk_leads_detail(name,email,mobile_no,alternative_no,created_at) VALUES (?,?,?,?,?)");
                	$query->bindValue(1, $data['2']);
					$query->bindValue(2, $data['1']);
					$query->bindValue(3, $data['3']);
					$query->bindValue(4, $data['4']);
    ...
    public function dsk_leads_mass_delete($data)
    {
		for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("DELETE from dsk_leads_detail WHERE id=?");
			$query->bindValue(1,$id);
			try{
				$query->execute();
		...
		if(count($data)-1 == count($rowCount)){
			return true;
		}else{
			return false;
		}
    ...
    public function dsk_leads_mass_move($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?,color_code=? WHERE id=?");
			$query->bindValue(1,'1');
			$query->bindValue(2,'');
    ...
    public function getALLDskLeadsPhoto()
    {
    	$query = $this->db->prepare("SELECT * FROM dsk_leads_detail  WHERE photo_received = '1' ORDER BY created_at DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function dsk_leads_mass_move_payment($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET 2nd_payment=?,color_code=? WHERE id=?");
			$query->bindValue(1,'1');
			$query->bindValue(2,'');
			$query->bindValue(3,$id);
			try{
    ...
    public function getALLDskPaymentReceivedLeads()
    {
    	$query = $this->db->prepare("SELECT * FROM dsk_leads_detail  WHERE 2nd_payment = '1' ORDER BY created_at DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function dsk_leads_mass_cancelled($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?,color_code=? WHERE id=?");
			$query->bindValue(1,'1');
			$query->bindValue(2,'');
			$query->bindValue(3,$id);
			try{
    ...
    public function getALLDskCancelledLeads()
    {
    	$query = $this->db->prepare("SELECT * FROM dsk_leads_detail  WHERE cancelled = '1' ORDER BY created_at DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function dsk_leads_mass_mail($data,$msg,$file)
    {
    	$realpath = '';
    	if ($file['error'] == 0) {
    		$filename = $file['name'];
	    	...
	    	$realpath =  'images/attchment/'.$filename;
	    	if (file_exists($realpath)) 
	    	{
	        	$filename = date("mdYHis")."_".$filename;
	        	$realpath = 'images/attchment/'.$filename;
	    	}
    	...
    	for ($i=1; $i <count($data) ; $i++) {
    		$id = $data[$i]['value'];
    		$query=$this->db->prepare("SELECT email FROM dsk_leads_detail WHERE id=?");
    		$query->bindValue(1,$id);
    		try{
    			$query->execute();
    			...
    			$to = $email['email'];
				$mail->setFrom('info@dev2.bluethink.in', 'DSK Leads');
				$mail->addAddress($to);
				if ($realpath != '') {
					$mail->addAttachment($realpath);
				}
				$mail->isHTML(true);                                  // Set email format to HTML
        		$mail->Subject = 'DSK MAIL';
        		$mail->Body    = $msg;
    			$j = 0;
    			if($mail->send()){
    				$sent[] = $j;
    ...
    public function getALLDskSuccessEvent()
    {
    	$query = $this->db->prepare("SELECT * FROM star_kid_event WHERE payment_status='success' ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function getALLMrsCapitalSuccess()
    {
    	$query = $this->db->prepare("SELECT * FROM mrs_capital WHERE payment_status='success' ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function getALLMrsCapitalFailed()
    {
    	$query = $this->db->prepare("SELECT * FROM mrs_capital WHERE payment_status='failure' ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function getALLMrsCapitalOthers()
    {
    	$query = $this->db->prepare("SELECT * FROM mrs_capital WHERE payment_status='' ORDER BY id DESC");
		try{
			$query->execute();
			return $query->fetchAll(PDO::FETCH_ASSOC);
		} catch(PDOException $e){
    ...
    public function dsk_leads_move_photo($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?,color_code=? WHERE id=?");
		$query->bindValue(1,'1');
		$query->bindValue(2,'');
    ...
    public function dsk_leads_move_payment($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET 2nd_payment=?,color_code=? WHERE id=?");
		$query->bindValue(1,'1');
		$query->bindValue(2,'');
    ...
    public function move_individual_photo_to_leads($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?, color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'');
    ...
    public function move_individual_photo_to_payment($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?, 2nd_payment=?, color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'1');
		$query->bindValue(3,'');
    ...
    public function move_individual_payment_to_leads($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET 2nd_payment=?, color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'');
    ...
    public function move_individual_payment_to_photo($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET 2nd_payment=?,photo_received=?,color_code WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'1');
		$query->bindValue(3,'');
		$query->bindValue(4,$id);
    ...
    public function move_individual_cancel_to_photo($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?,photo_received=?,color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'1');
		$query->bindValue(3,'');
		$query->bindValue(4,$id);
    ...
    public function move_individual_cancel_to_payment($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?,2nd_payment=?,color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'1');
		$query->bindValue(3,'');
		$query->bindValue(4,$id);
    ...
    public function move_individual_cancel_to_leads($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?,color_code=? WHERE id=?");
		$query->bindValue(1,'0');
		$query->bindValue(2,'');
    ...
    public function move_individual_leads_to_photo($id)
    {
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?,color_code=? WHERE id=?");
		$query->bindValue(1,'1');
		$query->bindValue(2,'');
    ...
    public function dsk_leads_cancel_data($data)
    {
    	extract($data);
		$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?, cancel_reason=?,photo_received=?,2nd_payment=? WHERE id=?");
		$query->bindValue(1,'1');
		$query->bindValue(2,$creason);
		$query->bindValue(3,'0');
		$query->bindValue(4,'0');
		$query->bindValue(5,$leadid);
		try{
			$query->execute();
			return true;
		}catch(PDOException $e){
    ...
    public function getExportLeads($action)
    {
		if ($action == 'dsk_event_export') {
    		$query = $this->db->prepare("SELECT id,parent_fname,email,relationship,mobile_no,alternative_no,address,city,state,zip,child_fname,age,payment_status,amount FROM star_kid_event ORDER BY id ASC");
    	}
		if ($action == 'payement_success') {
    		$query = $this->db->prepare("SELECT id,parent_fname,email,relationship,mobile_no,alternative_no,child_fname,age,payment_status,amount FROM star_kid_event WHERE payment_status='success' ORDER BY id DESC");
    	}
    	if ($action == 'exportleads') {
    		$query = $this->db->prepare("SELECT id,name,email,mobile_no,alternative_no,created_at FROM dsk_leads_detail  WHERE (photo_received = '0' AND 2nd_payment = '0' AND cancelled = '0') ORDER BY id DESC");
    	}else if ($action == 'exportphoto') {
    		$query = $this->db->prepare("SELECT id,name,email,mobile_no,alternative_no,created_at FROM dsk_leads_detail  WHERE photo_received = '1' ORDER BY created_at DESC");
    	}else if ($action == 'export2ndPayment') {
    		$query = $this->db->prepare("SELECT id,name,email,mobile_no,alternative_no,created_at FROM dsk_leads_detail  WHERE 2nd_payment = '1' ORDER BY created_at DESC");
    	}else if ($action == 'exportcancelled') {
    		$query = $this->db->prepare("SELECT * FROM dsk_leads_detail  WHERE cancelled = '1' ORDER BY created_at DESC");
    	}
    ...
    public function send_individual_mail($id,$msg,$file)
    {
    	$realpath = '';
    	if ($file['error'] == 0) {
    		$filename = $file['name'];
		...
		try{
			...
			$email = $query->fetch(PDO::FETCH_ASSOC);
			$to = $email['email'];
			$mail->setFrom('dskevent@software.lidtravel.com', 'DSK Leads');
			$mail->addAddress($to);
			if ($realpath != '') {
				$mail->addAttachment($realpath);
			}
    ...
    public function massLeads($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET photo_received=?,color_code=? WHERE id=?");
    ...
    public function massFromPayment($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET 2nd_payment=?,color_code=? WHERE id=?");
    ...
    public function massFromCancelled($data)
    {
    	for ($i=1; $i <count($data) ; $i++) { 
			$id = $data[$i]['value'];
			$query=$this->db->prepare("UPDATE dsk_leads_detail SET cancelled=?,cancel_reason=?,color_code=? WHERE id=?");
			$query->bindValue(1,'0');
			$query->bindValue(2,'');
			$query->bindValue(3,'');
			$query->bindValue(4,$id);
    ...
    public function change_dsk_row_color($post)
    {
    	extract($post);
    	$query=$this->db->prepare("SELECT created_at FROM dsk_leads_detail WHERE id=?");
    	$query->bindValue(1,$leadid);
    	try{
			$query->execute();
			$created_at = $query->fetch(PDO::FETCH_ASSOC);
			$date =$created_at['created_at'];
		}catch(PDOException $e){
				die($e->getMessage());
		}
    	$query=$this->db->prepare("UPDATE dsk_leads_detail SET color_code=?, created_at=? WHERE id=?");
    	$query->bindValue(1,$code);
    	$query->bindValue(2,$date);
		$query->bindValue(3,$leadid);
		try{
			$query->execute();
			return true;
		}catch(PDOException $e){
    ...
    public function dsk_leads_mass_change_color($data,$colorcode)
    {
    	for ($i=1; $i <count($data) ; $i++) {
    		$id = $data[$i]['value'];
    		$query=$this->db->prepare("SELECT created_at FROM dsk_leads_detail WHERE id=?");
	...
	{
		$query = $this->db->prepare("SELECT * FROM partner_register WHERE id=?");
		$query->bindValue(1, $id);
		...
		public function add_partner($partnerData)
	{
		extract($partnerData);
		/*date_default_timezone_set('Asia/Kolkata');
		$qDate=date('Y-m-d h:i:s');*/
		if(!empty($partnerId))
		{
			$query=$this->db->prepare("UPDATE  partner_register SET partner_name=?, contact_no=?, email=?, password = ?, address=?, url_link=?, hotel_markup=?, package_markup=?, user_name=? WHERE id=?");
			
			$query->bindValue(1, $partnerName);
			$query->bindValue(2, $contactNum);
			$query->bindValue(3, $partnerEmail);
			$query->bindValue(4, $password);
			$query->bindValue(5, $address);
			$query->bindValue(6, $urlLink);
			$query->bindValue(7, $markupHotel);
			$query->bindValue(8, $markupPackage);
			$query->bindValue(9, $userName);
			$query->bindValue(10, $partnerId);
		}
		else
		{
			/*$queryGet=$this->db->prepare("SELECT id FROM  partner_register ORDER BY id DESC LIMIT 1");
			$queryGet->execute();
			$data = $queryGet->fetch(PDO::FETCH_ASSOC);
			$num = $data['id']+1;
			$queryNum = 'lidq00'.$num;
			...
			$query=$this->db->prepare("INSERT INTO  partner_register(partner_name, contact_no, email, password, address, url_link, hotel_markup, package_markup, user_name,c_type) VALUES(?,?,?,?,?,?,?,?,?,?)");
			...
			/*$query->bindValue(9, $partnerId);*/
			/*$query->bindValue(10, $addedBy);*/
		}
	...
	public function deletePartnerById($id){
		$query=$this->db->prepare("DELETE from partner_register WHERE id=?");
		$query->bindValue(1,$id);
		try{
			$query->execute(); 
			return true;
	...
	public function partner_list() {
		$query = $this->db->prepare("SELECT * FROM partner_register");
   ...
   public function save_tc_edit_details($queryData2)
	{
		extract($queryData2);
		
		//print_r($queryData2);exit;
		$country2=implode(",",$country);
		$state1=implode(",",$state);
		$city1=implode(",",$city);
		//$package_search=$_POST['package_search'];
		if($nameSearch=="hotel names")
		{
		if($choosen_pack!="")
		{
		$package_id1=explode("&",$choosen_pack);	
		}
		else
		{   
			if($package_search == "hotel names")
			{
			$package_id1=explode("&",$package_id);
			}
			else
			{
				$package_id1=explode("&",$package_search);
			}
		}
		$pack_epo1=$package_id1[0].'&'.$package_id1[1];
		$search_data = base64_decode($package_id1[1]);
		$pack_epo2=$package_id1[0];	
		}
		else
		{
		  $pack_epo=explode("&",$package_search);
		  $pack_epo1=$pack_epo[0].'&'.$pack_epo[1];
		  $search_data = base64_decode($pack_epo[1]);
		  $pack_epo2=$pack_epo[0];
		}
		if($queryType=='Hotel')
		{
			$grandPackageTotal2=$grand_total_hotel;
			$pricemargin_package1=$pricemargin_hotel;
		}
		else
		{
			$grandPackageTotal2=$grandPackageTotal1;
			$pricemargin_package1=$pricemargin_package;
		}
		...
		if($nameSearch=="hotel names")
		{
		$adults1=$paramArr['adults'];
		$child1=$paramArr['child'];
		$child_age1=$paramArr['child_age'];
		$userSelectRoomTypes6=$userSelectRoomTypes1;
			$userSelectMealTypes6=$userSelectMealTypes1;
		
		}
		else
		{
			$userSelectRoomTypes6=$userSelectRoomTypes5;
			$userSelectMealTypes6=$userSelectMealTypes5;
			$adults1=$adults;
		$child1=$child;
		$child_age1=$child_age;
		...
		// echo  $query="UPDATE tour_card SET bkg_date = '$bkg_date', bkg_by = '$bkg_by', queryType = $queryType, name_prefix = '$name_prefix', pax_name = '$pax_name', country= '$country1', party='$party' ,file_no='$file_no', invoice_no='$invoice_no'  WHERE id = '$package_id_page'";exit;
		
		$query=$this->db->prepare("UPDATE tour_card SET bkg_date = ?, bkg_by = ?, queryType = ?, name_prefix = ?, pax_name = ?, country= ?, party=? ,file_no=?, invoice_no=?  WHERE id = ?");
		$query->bindvalue(1, $bkg_date);
		$query->bindValue(2, $bkg_by);
		$query->bindValue(3, $queryType);
		$query->bindValue(4, $name_prefix);
		$query->bindValue(5, $pax_name);
		$query->bindValue(6, $country1);
		$query->bindValue(7, $party);
		$query->bindValue(8, $file_no);
		$query->bindValue(9, $invoice_no);
		$query->bindValue(10, $package_id_page);
        $query->execute(); 
		
		
		$query=$this->db->prepare("UPDATE tour_card_details SET tour_card_id = ?, country = ?, state = ?, city = ?, check_in = ?, check_out= ?, nights=? ,choosen_pack=?, vehicle_name=?, vehicle_package_cost=?,no_of_package_vehicle=?,margin_percent=?,grand_total=? WHERE tour_card_id = ?");
		$query->bindvalue(1, $package_id_page);
		$query->bindValue(2, $country2);
		$query->bindValue(3, $state1);
		$query->bindValue(4, $city1);
		$query->bindValue(5, $startdate);
		$query->bindValue(6, $enddate);
		$query->bindValue(7, $stayDuration);
		$query->bindValue(8, $pack_epo1);
		$query->bindValue(9, $vehicle_name);
		$query->bindValue(10, $vehicle_package_cost);
		$query->bindValue(11, $no_of_package_vehicle);
		$query->bindValue(12, $pricemargin_package1);
		$query->bindValue(13, $grandPackageTotal2);
		$query->bindValue(14, $package_id_page);
        $query->execute(); 
		...
		if($queryType!='Package')
			{
				$query_hotel_del = $this->db->prepare("DELETE FROM tour_card_room_dt WHERE tour_card_id  = ?");
		$query_hotel_del->bindValue(1, $package_id_page);
		$query_hotel_del->execute();
            for($i=0; $i<count($adults1); $i++)
										{
											$adultP = $adults1[$i];
											$childp = $child1[$i];
											
												$childp_age = $child_age1[$i];
											...
											$hotel_selected_rooms1=explode(",",$hotel_selected_rooms);
											$hotel_selected_rooms2=$hotel_selected_rooms1[$i];
											
											$userSelectRoomTypes2=explode(",",$userSelectRoomTypes6);
											$userSelectRoomTypes3=$userSelectRoomTypes2[$i];
											
											
											$hotel_meal_plans1=explode(",",$hotel_meal_plans);
											$hotel_meal_plans2=$hotel_meal_plans1[$i];
											
											$userSelectMealTypes2=explode(",",$userSelectMealTypes6);
											$userSelectMealTypes3=$userSelectMealTypes2[$i];
											
											$calculated_hotel_prices1=json_decode($calculated_hotel_prices, true);
											$calculated_hotel_prices2=$calculated_hotel_prices1[$i+1];
											$c2=$calculated_hotel_prices2*$pricemargin_hotel/100;
											$c3=$calculated_hotel_prices2+$c2;
			         ...
			         $a=$a+$i;
           			 ...
           			 // echo $query2 = "INSERT INTO tour_card_room_dt(tour_card_id,room, adult,children,child_age,room_type,meal_plan,price) VALUES('$tc_id','$a','$adultP','$childp','$childp_age','$hotel_selected_rooms2','$hotel_meal_plans2','$calculated_hotel_prices2')";
			
              $query2=$this->db->prepare("INSERT INTO tour_card_room_dt(tour_card_id,room, adult,children,child_age,room_type,room_type_id,meal_plan,meal_plan_id,price) VALUES(?,?,?,?,?,?,?,?,?,?)");
			
			$query2->bindValue(1, $package_id_page);
			$query2->bindValue(2, $a);
			$query2->bindValue(3, $adultP);
			$query2->bindValue(4, $childp);
			$query2->bindValue(5, $childp_age);
			$query2->bindValue(6, $hotel_selected_rooms2);
			$query2->bindValue(7, $userSelectRoomTypes3);
			$query2->bindValue(8, $hotel_meal_plans2);
			$query2->bindValue(9, $userSelectMealTypes3);
			$query2->bindValue(10, $c3);
			$query2->execute(); 
			 $a++;
			...
			{
				$query_city_del = $this->db->prepare("DELETE FROM tour_card_package_dt WHERE tour_card_id  = ?");
		$query_city_del->bindValue(1, $package_id_page);
		$query_city_del->execute();
		$stay_duration1=$stayDuration+1;
				for($c=1; $c<=$stay_duration1; $c++)
				 {
					     
					    $query_city = $this->db->prepare("SELECT * FROM itinerary_management WHERE id = ?");
						$query_city->bindValue(1, $pack_epo2);
						$query_city->execute();
						$itineraryData=$query_city->fetch(PDO::FETCH_ASSOC);
						$duration_city = $itineraryData['duration_city'];
					    $durationCityArr = explode(',',$duration_city);
						$cityId = $durationCityArr[$c-1];
						$query_city1 = $this->db->prepare("SELECT * FROM citymaster WHERE id IN ($cityId)");
						$query_city1->execute();
			            $cityArr=$query_city1->fetchAll(PDO::FETCH_ASSOC);
						$cityArr1=$cityArr[0]['city'];
						$day = '+'.($c-1).' day';
						$day1=0;
						$day1=$day1+$c;
						...
						$date = date('d, F Y',strtotime($date1 . $day));
						$dateSearch = date('Y-m-d',strtotime($date1 . $day));
						
						$selected_package_hotels1=explode("$#$",$selected_package_hotels);	
						$selected_package_hotels3=$selected_package_hotels1[1];
						
						
						$selected_package_rooms2=explode("$#$",$selected_package_rooms1);
						$paramArr = array();
						foreach($selected_package_rooms2 as $data)
							{
								$paramArr[]=$data;
							}
		                      
						$selected_package_rooms3= $paramArr;
						$selected_package_rooms4=$selected_package_rooms3[1];
						
						//echo $selected_package_mealPlans1[$c];
						
						$selected_package_mealPlans2=explode(",",$selected_package_mealPlans1);
						$selected_package_mealPlans3=$selected_package_mealPlans2[$c-1];
						
						
						$selected_package_mealPlans1_name2=explode(",",$selected_package_mealPlans1_name);
						$selected_package_mealPlans1_name3=$selected_package_mealPlans1_name2[$c-1];
						
						// echo $query2="INSERT INTO tour_card_package_dt(tour_card_id,Day, City,pck_date,hotel,room_type,meal_paln) VALUES('$tc_id','$day1','$cityArr1','$dateSearch','$selected_package_hotels3','$selected_package_rooms4','$selected_package_mealPlans3')";
						
						$query2=$this->db->prepare("INSERT INTO tour_card_package_dt(tour_card_id,Day, City,pck_date,hotel,room_type,meal_paln,meal_plan_name) VALUES(?,?,?,?,?,?,?,?)");
			
			$query2->bindValue(1, $package_id_page);
			$query2->bindValue(2, $day1);
			$query2->bindValue(3, $cityArr1);
			$query2->bindValue(4, $dateSearch);
			$query2->bindValue(5, $selected_package_hotels3);
			$query2->bindValue(6, $selected_package_rooms4);
			$query2->bindValue(7, $selected_package_mealPlans3);
			$query2->bindValue(8, $selected_package_mealPlans1_name3);
			$query2->execute(); 
					$day1++;
				 }
				 $query_adult_del = $this->db->prepare("DELETE FROM tour_card_room_dt WHERE tour_card_id  = ?");
		$query_adult_del->bindValue(1, $package_id_page);
		$query_adult_del->execute();
				 ...
				 for($i=0; $i<count($adults1); $i++)
										{
											...
											$calculate_package_price1=json_decode($calculate_package_price, true);
											$calculate_package_price2=$calculate_package_price1[$i+1];
              ...
              $query4=$this->db->prepare("INSERT INTO tour_card_room_dt(tour_card_id,room, adult,children,child_age,price) VALUES(?,?,?,?,?,?)");
			
			$query4->bindValue(1, $package_id_page);
			$query4->bindValue(2, $a);
			$query4->bindValue(3, $adultP);
			$query4->bindValue(4, $childp);
			$query4->bindValue(5, $childp_age);
			$query4->bindValue(6, $calculate_package_price2);
			$query4->execute(); 
			 $a++;
     ...
     public function other_edit_tour_card_details($post)
	{
		 /*echo "<pre>@@@@";
		 print_r($post);exit;*/
		extract($post);
		//print_r($_POST);exit;
		$query = $this->db->prepare("UPDATE other_tour_card_details SET bkg_date=?,name_prefix=?,pax_name=?,country=?,party=?,file_no=?,invoice_no=?,adult=?,children=?,child_age=?,city=?,particulars=?,hotel_filter=?,from_date=?,to_date=?,hotel=?,price=?,gst=?,margin=?,cgst=?,gst_percent=?,gst_amount=? WHERE id=?");
		...
		else
		{
			...
			$query->bindValue(3, $pax_name);
			$query->bindValue(4, $country1);
			$query->bindValue(5, $party);
			$query->bindValue(6, $file_no);
			$query->bindValue(7, $invoice_no);
			$query->bindValue(8, $adults);
			$query->bindValue(9, $kids);
			$query->bindValue(10, $kids_age);
			$query->bindValue(11, json_encode($other_destination));
			$query->bindValue(12, json_encode($other_particulars));
            $query->bindValue(13, json_encode($hotel_filter));
			$query->bindValue(14, json_encode($from_date));
			$query->bindValue(15, json_encode($to_date));
			$query->bindValue(16, json_encode($partner_hotel));
			$query->bindValue(17, json_encode($other_cost));
			$query->bindValue(18, $gst);
			$query->bindValue(19, $other_margin);
			$query->bindValue(20, $cgst);
			$query->bindValue(21, $cgst2);
			$query->bindValue(22, $gst_amount);
			$query->bindValue(23, $page_id);
			try{
			$query->execute();
            return true;
		    } catch(PDOException $e){
	...
	public function getAllHotelConfirmation($start_from,$record_per_page)
	{
		$query = $this->db->prepare("SELECT * FROM hotel_confrm_details  ORDER BY id DESC LIMIT $start_from, $record_per_page");	
	...
	public function editAllHotelConfirmation($id)
	{
		$query = $this->db->prepare("SELECT * FROM hotel_confrm_details WHERE id=?");	
		$query->bindValue(1, $id);
		try{
			$query->execute();
			return $query->fetch(PDO::FETCH_ASSOC);
