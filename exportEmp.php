...
include('config/init.php');
	...
	$exportData=$objAdmin->user_list();
	
	$rowCol1=$objAdmin->getColName('admin',$db);
	...
	$rowCol = array_merge($rowCol1,$rowCol2,$rowCol3,$rowCol4);
	foreach($rowCol as $key=>$val)
	{
		...
		if($val['column_name'] == 'password')
		{
			$colName='Password';
			$header .= $colName."\t";
		}
		/* if($val['column_name'] == 'designation_name')
		{
			$colName='Designation';
			$header .= $colName."\t";
		} */
		/* if($val['column_name'] == 'datetime')
		{
			$colName='Date';
			$header .= $colName."\t";
		} */
