<?php
    ...
    $arrQuery=$objAdmin->getALLDskPaymentReceivedLeads();
    ...
    $("[data-toggle=tooltip]").tooltip();
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
              2nd Payment Received Leads Detail
          </h1>
          <div id="status"></div>
          
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> DSK EVENT</a></li>
            <li class="active">2nd Payment Received List</li>
          </ol>
        </section>
  ...
  <div class="modal-dialog">
    ...
    function DeleteAll() {
      ...
      if (confirm == false) {
        return false;
            ...
            if(result == '1')
            {
              window.location.href='dsk_second_payment.php';
            }
            else if (result == '0')
            {
      ...
      if (confirm == false) {
        return false;
             ...
             var ids =   $("#massActionFrom").serializeArray();
              $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=fromPayment',
              data:{leadid:ids},
              success:function(result) {
              if(result == '1')
              {
    ...
    function Move() {
        ...
        if (moveto == 'photo_received') {
            $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=move_payment_to_photo',
              data:{leadid:id},
              success:function(result) {
              if(result == '1')
              {
        ...
        }else if (moveto == 'leads') {
             $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=move_payment_to_leads',
              data:{leadid:id},
              success:function(result) {
              if(result == '1')
              {
    ...
    function Export() {
      var conf = confirm("Export Leads to CSV?");
        if(conf == true)
        {
          window.open("exportleads.php?action=export2ndPayment", '_blank');
        }
    }
