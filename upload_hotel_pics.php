...
// }
 if(is_array($_FILES))  
 {  	
      foreach($_FILES['Pics']['name'] as $name => $value)  
      {  
           $file_name = explode(".", $_FILES['Pics']['name'][$name]);  
           $allowed_extension = array("jpg", "jpeg", "png", "gif");  
           if(in_array($file_name[1], $allowed_extension))  
           {  
                $new_name = rand() . '.'. $file_name[1];  
                $sourcePath = $_FILES["Pics"]["tmp_name"][$name];  
                $targetPath = "document/hotel_doc/hotel_room_pic/".$new_name;  
