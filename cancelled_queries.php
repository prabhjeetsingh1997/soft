<?php 
	...
	if($_GET['action']=='delete')
	{
		...
		header("location:cancelled_queries.php");
	} 
	//print_r($_SESSION);
	...
	$arrQuery=$objAdmin->getAllCancelledQuery($user_type, $empType, $queryType,$start_from,$record_per_page,$user_id);
          ...
          <div class="row">
            <div class="col-xs-12">
               ...
               <!--  <div class="box-header">
                  ...
                  <table id="example1" class="table table-bordered table-striped">
					...
					?>
                      <tr>
						...
						<td>
							<div id="cdetal_<?php echo $val['query_no'];?>" style="display:none;"><?php echo $val['cancel_reason'];?></div>
							
							<button type="button" rel="<?php echo $val['query_no'];?>" class="btn btn-info btn-sm cancelDetail" style="background-color: #337ab7;"><i class="fa fa-eye"></i></button>
						</td>
                        <td>
							<?php 
								$text = 'Reply Pending';
						...
						<td>
							<div class="btn-group btn-group-xs">
								...
								<a class="btn btn-info" title="Undo Cancelled Query" data-toggle="tooltip" style="background-color: #0f557d;" onclick="undoCanQuery('<?php echo $val['id'];?>');"><i class="fa fa-undo" aria-hidden="true"></i></i></a>
								<?php
    			...
    			<br />
    		<?php
			    $page_result=$objAdmin->getAllCancelledQueryResult($user_type, $empType, $queryType,$user_id);
			    ...
			    if($page > 1)
			    {
			     ...
			     echo "<li class='queryPage'><a href='cancelled_queries.php?page=1'>First</a></li>";
			     echo "<li class='queryPage'><a href='cancelled_queries.php?page=".($page - 1)."'><<</a></li>";
			    }
			    for($i=$start_loop; $i<=$end_loop; $i++)
			    {
			     ...
			     echo "<li class='queryPage ".$class."'><a href='cancelled_queries.php?page=".$i."'>".$i."</a></li>";
			    }
			    if($page <= $end_loop)
			    {
			     echo "<li class='queryPage'><a href='cancelled_queries.php?page=".($page + 1)."'>>></a></li>";
			     echo "<li class='queryPage'><a href='cancelled_queries.php?page=".$total_pages."'>Last</a></li>";
			    }
			    echo "</ul>";
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      $(function () {
		...
		$(document).on('click',".cancelDetail",function(){
			$("#cDetail").modal();
			var qn = $(this).attr('rel');
			$("#modalcancelHead").html('Cancellation Detail: <strong>'+qn+'</strong>');
			$("#cdetail").html($("#cdetal_"+qn).html());
...
<script type="text/javascript">
	function undoCanQuery(query_id) {
		$.ajax({
			type:'post',
			url :'_ajax_query_undocancel_request.php',
			data:{id:query_id},
			beforeSend:function() {
			},
			success:function(result) {
				...
				{
					$("#status").show().html('<div class="alert alert-danger">Sorry, Undo Cancel Query request not generated</div>');
