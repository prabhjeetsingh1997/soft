<?php
    ...
    $arrQuery=$objAdmin->getALLDskCancelledLeads();
    ...
    $("[data-toggle=tooltip]").tooltip();
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
              All Cancelled DSK Leads Detail
          </h1>
          <div id="status"></div>
          
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> DSK EVENT</a></li>
            <li class="active">cancelled List</li>
          </ol>
        </section>
        ...
        <ul class="dropdown-menu multi-level" role="menu" aria-labelledby="dropdownMenu">
              ...
              <li class="dropdown-submenu">
                ...
                <ul class="dropdown-menu">
                  <li><a tabindex="-1" href="#" onclick="Leads();">Leads</a></li>
                  <li class="divider"></li>
                  <li><a href="#" onclick="photoReceived();">Photo Received</a></li> <li class="divider"></li>               
...
<div id="masscolorDetail" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    function DeleteAll() {
      ...
      if (confirm == false) {
        return false;
            ...
            if(result == '1')
            {
              window.location.href='dsk_leads_cancelled.php';
            }
            else if (result == '0')
            {
      ...
      if (confirm == false) {
        return false;
             ...
             var ids =   $("#massActionFrom").serializeArray();
              $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=massMove',
              data:{leadid:ids},
              success:function(result) {
              if(result == '1')
              {
      ...
      if (confirm == false) {
        return false;
             ...
             var ids =   $("#massActionFrom").serializeArray();
              $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=fromCancelled',
              data:{leadid:ids},
              success:function(result) {
              if(result == '1')
              {
    ...
    function Move() {
        ...
        if (moveto == 'photo_received') {
            $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=move_cancel_to_photo',
              data:{leadid:id},
              success:function(result) {
              if(result == '1')
              {
             ...
             $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=move_cancel_to_payment',
              data:{leadid:id},
              success:function(result) {
              if(result == '1')
              {
          ...
          $.ajax({
              type:'post',
              url: '_ajax_dsk_lead_massaction.php?action=move_cancel_to_leads',
              data:{leadid:id},
              success:function(result) {
              if(result == '1')
              {
    ...
    function Export() {
      var conf = confirm("Export Leads to CSV?");
        if(conf == true)
        {
          window.open("exportleads.php?action=exportcancelled", '_blank');
        }
    }
